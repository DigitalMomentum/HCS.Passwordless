# HCS.Umbraco.Passwordless.WebAuthn — CLAUDE.md
...
| `WebAuthn.Endpoints` | Sign-in and registration endpoints (4 flow endpoints + credential management) |
...
- Counter regression detection is not optional. When `SignInCompleteEndpoint` detects a regressed counter, it **must** fire `PasskeyCounterRegressionNotification` before completing sign-in.
...
- Counter regression check in `SignInCompleteEndpoint` — this is a FIDO2 spec requirement.
