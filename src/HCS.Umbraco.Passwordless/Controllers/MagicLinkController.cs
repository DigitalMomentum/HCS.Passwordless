...
public partial class MagicLinkController : UmbracoApiController
{
    ...
    public async Task<IActionResult> SendLink([FromBody] RequestByEmailDto dto, CancellationToken ct)
    {
        ...
        if (!options.MagicLink.Enabled) return NotFound();
        ...
        if (member is not null)
        {
            ...
            await _sender.SendMagicLinkAsync(member, link, options.MagicLink.TokenLifespan, ct);
        }
        else
        {
        ...
        CancellationToken ct)
    {
        ...
        if (options.MagicLink.SingleUse)
        {
            var isFirstUse = await _singleUse.TryMarkUsedAsync(tokenHash, options.MagicLink.TokenLifespan, ct);
